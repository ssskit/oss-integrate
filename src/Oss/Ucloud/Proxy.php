...
class Proxy
{
    ...
    function UCloud_PutFile($bucket, $key, $file)
    {
        ...
        list($data, $err) =  (new Http())->UCloud_Client_Call($client, $req);
        fclose($f);
...
//------------------------------表单上传------------------------------
    function UCloud_MultipartForm($bucket, $key, $file)
    {
        ...
        $req->Header['Expect'] = '';
        $token = (new Digest())->UCloud_SignRequest(null, $req, $mimetype);

        $fields = array('Authorization'=>$token, 'FileName' => $key);
        $files  = array('files'=>array('file', $file, $content, $mimetype));

        $client = new UCloud_AuthHttpClient(null, Config::NO_AUTH_CHECK);
        list($data, $err) = (new Http())->UCloud_Client_CallWithMultipartForm($client, $req, $fields, $files);
        fclose($f);
        return array($data, $err);
    }
...
//------------------------------分片上传------------------------------
    function UCloud_MInit($bucket, $key)
    {
        ...
        return (new Http())->UCloud_Client_Call($client, $req);
    }
...
//------------------------------追加上传------------------------------
    function UCloud_AppendFile($bucket, $key, $file, $position)
    {
        ...
        $err = Utils::CheckConfig(ActionType::APPENDFILE);
        if ($err != null) {
            return array(null, $err);
        }
...
// @results: $url
    function UCloud_MakePrivateUrl($bucket, $key, $expires = 0)
    {
        ...
        $client = new UCloud_AuthHttpClient(null);
        $temp = $client->Auth->SignRequest($req, null, Config::QUERY_STRING_CHECK);
